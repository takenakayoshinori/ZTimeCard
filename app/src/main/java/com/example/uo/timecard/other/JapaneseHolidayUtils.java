...
public class JapaneseHolidayUtils {
    ...
    /**
     * 指定した日付のNationalHolidayオブジェクトを取得します。
     * 指定した日付が「国民の祝日」ではない場合はnullを返します。
     *
     * @param cal    日付
     * @return 指定した日付のNationalHolidayオブジェクト
     * @throws NullPointerException    calがnullの場合
     */
     ...
     * 「国民の祝日」、「振替休日」、「国民の休日」のいずれでもない場合はnullを返します。
     *
     * @param cal    日付
    ...
    public static void main(String[] argv){
        ...
        System.out.println("-----2018年の国民の祝日-----");
        cal.set(2018, 0, 1);
        for (int i = 0; i < 365; i++) {
            if (JapaneseHolidayUtils.isNationalHoliday(cal)) {
                String date = cal.get(Calendar.YEAR) + "/" +
                        (cal.get(Calendar.MONTH) + 1) + "/" +
                        cal.get(Calendar.DAY_OF_MONTH);
                System.out.println(date + "  " + JapaneseHolidayUtils.getHolidayName(cal));
            }
            cal.add(Calendar.DAY_OF_YEAR, 1);    // 一日進める
        }

        System.out.println();

        System.out.println("-----2018年の振替休日-----");
        cal.set(2018, 0, 1);
        for (int i = 0; i < 365; i++) {
            if (JapaneseHolidayUtils.isSubstituteHoliday(cal)) {
                String date = cal.get(Calendar.YEAR) + "/" +
                        (cal.get(Calendar.MONTH) + 1) + "/" +
                        cal.get(Calendar.DAY_OF_MONTH);
                System.out.println(date + "  " + JapaneseHolidayUtils.getHolidayName(cal));
        ...
        System.out.println("-----2018年の国民の休日-----");
        cal.set(2018, 0, 1);
        for (int i = 0; i < 365; i++) {
            if (JapaneseHolidayUtils.isCitizensHoliday(cal)) {
                String date = cal.get(Calendar.YEAR) + "/" +
                        (cal.get(Calendar.MONTH) + 1) + "/" +
                        cal.get(Calendar.DAY_OF_MONTH);
                System.out.println(date + "  " + JapaneseHolidayUtils.getHolidayName(cal));
